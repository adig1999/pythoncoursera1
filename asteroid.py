...
class Ship:
    ...
    def update(self):
        ...
        fric=0.991
        curr_vec=angle_to_vector(self.angle)
        acc=0.05
        self.vel[0] *= (fric)
        self.vel[1] *= (fric)
        if self.thrust:
            self.vel[0]+=curr_vec[0]*acc
            self.vel[1]+=curr_vec[1]*acc


    def increase_an(self):
        self.angle_vel-=0.05

    def decrease_an(self):
        self.angle_vel+=0.05

    def update_thrust(self,curr):
        self.thrust = curr
        if self.thrust:
    ...
    def shoot(self):
        ...
        missile_velocity = [self.vel[0] + forward[0] *5, self.vel[1] + forward[1] *5]
...
def rock_spawner():
    global a_rock
    wd=random.randrange(0,WIDTH)
    ht=random.randrange(0,HEIGHT)
    v=[random.randint(0,100)*0.02,random.randint(0,100)*0.02]
    ang_v=random.randint(-50,50)*0.001
    a_rock = Sprite([wd,ht], v, 0, ang_v, asteroid_image, asteroid_info)
